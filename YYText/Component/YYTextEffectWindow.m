        ...
        if (!mag.snapshot || mag.snapshot.size.width > 1) {
            ...
            dispatch_once(&onceToken, ^{
                ...
                if (@available(iOS 10.0, *)) {
                    placeholder = [UIGraphicsImageRenderer yy_render:rect.size opaque:NO scale:0 actions:^(UIGraphicsImageRendererContext * _Nonnull rendererContext) {
...
//    return rotation;
    ...
    if (@available(iOS 10.0, *)) {
        UIImage *image = [UIGraphicsImageRenderer yy_render:captureRect.size opaque:NO scale:0 actions:^(UIGraphicsImageRendererContext * _Nonnull rendererContext) {

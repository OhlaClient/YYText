    ...
    if (async) {
        ...
        dispatch_async(YYTextAsyncLayerGetDisplayQueue(), ^{
            ...
            if (@available(iOS 10.0, *)) {
                image = [UIGraphicsImageRenderer yy_render:size opaque:opaque scale:scale actions:^(UIGraphicsImageRendererContext * _Nonnull rendererContext) {
...
//            UIGraphicsEndImageContext();
        ...
        UIImage *image;
        if (@available(iOS 10.0, *)) {
            image = [UIGraphicsImageRenderer yy_render:self.bounds.size opaque:self.opaque scale:self.contentsScale actions:^(UIGraphicsImageRendererContext * _Nonnull rendererContext) {
                CGContextRef context = rendererContext.CGContext;
